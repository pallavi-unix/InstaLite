...
app.post("/photos/new", function (request, response) {
  ...
  processFormBody(request, response, function (err) {
    ...
    const { visibilityArray } = request.body;
    const visibleArray = visibilityArray.split(",");
      ...
      Photo.create(
          {
            ...
            comment: [],
            likes: [],
            visibility: visibleArray.map(user_id => ({ user_id })),
          })
          .then((returnValue) => {
            response.end();
          })
...
app.post("/like", async (request, response) => {
  try {
    const { like, post_id, user_id } = request.body;
    console.log( like, post_id, user_id, )
    
    if (like === undefined) {
      response.status(400).send("like required");
      return;
    }
    if(post_id === '') {
      response.status(400).send("post Id required");
      return;
    }
    const feedPost = await Photo.findById(post_id);
    if(like)
    {
        const like = {
          user_id, 
      ...
      const indexToRemove = feedPost.likes.findIndex(
        (likeItem) => likeItem.user_id === _id
      );
      console.log(indexToRemove, user_id)
      if (indexToRemove !== -1) {
        feedPost.likes.splice(indexToRemove, 1);
      }
      await feedPost.save();
    }
    response.json(feedPost.likes);
  } catch (er) {
    console.log(er)
  }
});
...
app.post('/favourite', async (request, response) => {
  const { favourite, fileName, user_id } = request.body;
  try {
    console.log(favourite, fileName, user_id)
    const user = await User.findById(user_id);
    if(favourite ==='ALL' || fileName === undefined) {
      response.status(200).send(user.favourites);
      console.log('all')
    }
    else if(favourite) {
      const file = {
      ...
      const indexToRemove = user.favourites.findIndex(
        (file) => file.file_name === fileName
      );
      console.log('Notfavourite')
      // console.log(indexToRemove, user_id)
      if (indexToRemove !== -1) {
...
// app.post('/visibility', (request, response) => {
//   const { allowedUsers, user_id } = req.body;
//   try {
 ...
 */
app.get("/photosOfUser/:id/:activeUser", function (request, response) {
  if (hasNoUserSession(request, response)) return;
  const id = request.params.id;
  const activeUser = request.params.activeUser;
  User.findById(id,{__v:0, login_name:0, password: 0})
      .then((user) => {
        if (user === null) {
          // Query didn't return an error but didn't find the SchemaInfo object -
